...
include "root" {
  path = "${dirname(find_in_parent_folders())}/_providers/terragrunt_azure_aks_tenant.hcl"
}
...
include "module" {
  path   = "${dirname(find_in_parent_folders())}/_modules/azure/logscale/terragrunt.hcl"
  expose = true
}
