...
include "module" {
  path   = "${dirname(find_in_parent_folders())}/_modules/azure/azure-dns/zone-sub/terragrunt.hcl"
  expose = true
}
...
dependency "vnet" {
  config_path = "../eastus/vnet/"  # Reference the env folder where the cluster state is stored
}
